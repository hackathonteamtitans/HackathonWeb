...
function butRowAcep(but) {
    ...
    IterarCamposEdit($cols, function ($td) {  //itera por la columnas
        ...
        else if (isAnchor($td.html())) {
            var href = $td.find('a').prop('href');
            cont = '<a href="' + href + '" class="alert-dismissible" target="_blank">Resume</a></td>';
        }
        else {
...
function butRowEdit(but) {
    ...
    IterarCamposEdit($cols, function ($td) {  //itera por la columnas
        ...
        if (isAnchor(cont)) {
            var href = $td.find('a').prop('href');
            input = '<input class="form-control input-sm"  value="' + href + '">';
        }
        else if (cont.trim().toLowerCase() == 'yes' || cont.trim().toLowerCase() == 'no')
        {
            if (cont.trim().toLowerCase() == 'yes') {
                input = '<select class="custom-select"><option Selected="True">Yes</option><option>No</option></select >';
            }
            else {
                input = '<select class="custom-select"><option>Yes</option><option Selected="True">No</option></select >';
            }
        }
        else {
...
function isAnchor(str) {
    return /<a[\s]+([^>]+)>((?:.(?!\<\/a\>))*.)<\/a>/g.test(str);
}
function isSelect(str) {
    return /<select[\s]+([^>]+)>((?:.(?!\<\/select\>))*.)<\/select>/g.test(str);
}
